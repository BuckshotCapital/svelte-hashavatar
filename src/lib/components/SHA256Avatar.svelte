<script lang="ts">
	...
	type Variants = 'normal' | 'stagger' | 'spider' | 'flower' | 'gem';

	type ColorMapper = (args: {
	...
	function generateSection({
		...
		const arcRadius =
			variant === 'gem' ? 0 : variant === 'flower' ? 0.25 * outerRadius : outerRadius;
		...
		return {
			path,
			transform: angleOffset !== 0 ? `rotate(${angleOffset.toFixed(6)}turn)` : undefined,
			label: {
				text: value,
				...polarPoint(
					innerRadius === 0 ? outerRadius * 0.66 : innerRadius + (outerRadius - innerRadius) / 2,
					angle
				)
			}
		};
	...
	let {
		radiusFactor = 0.42,
		hash = $bindable(Array(64).fill('0').join('')),
		showGrid = $bindable(false),
		showLabels = $bindable(false),
		showSections = $bindable(true),
		variant = $bindable('normal'),
		mapColor = $bindable(mapValueToColor),
		width = $bindable('16rem'),
		height = $bindable('16rem'),
		transition = $bindable(true),
		...props
	}: SHA256AvatarProps & SvelteHTMLElements['svg'] = $props();

	const mix = (a: number, b: number) => a * radiusFactor + b * (1 - radiusFactor);

	const r1 = $derived(variant === 'flower' ? 0.75 : 1);
	const r2 = $derived(mix((r1 * Math.sqrt(3)) / 2, r1 * 0.75));
	const r3 = $derived(mix((r1 * Math.sqrt(2)) / 2, r1 * 0.5));
	const r4 = $derived(mix(r1 * 0.5, r1 * 0.25));

	const bytes = $derived(hash?.match(/.{1,2}/g)?.map((block) => block) ?? []);
	const { soul, horcruxes } = $derived(getHashSoul(bytes));
	const bitCount = $derived(Math.round((hash?.length ?? 0) / 64)); // 32 sections = 64 hex characters

	const innerRadii = $derived([r2, r3, r4, 0]);
	const outerRadii = $derived([r1, r2, r3, r4]);
		...
		{#each sections as section, i (i)}
			<path
				...
				class={[
					...
					transition ? 'transition-transform duration-150 ease-out' : undefined
				]}
				stroke-width={0.02}
				stroke-linejoin="round"
				style="transform: {section.transform}"
			/>
		{/each}
	</g>
	{#if showLabels}
